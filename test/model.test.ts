...
test("probabilities match PyTorch on every synthetic decision", async () => {
  assert.equal(fx.source, manifest.source, "fixtures and model come from the same checkpoint");
  const session = await ort.InferenceSession.create(new URL(manifest.model, dir).pathname);
  const kev = new CuaS1({ ort: ort as unknown as OrtModule, session, manifest });
  let worst = 0, flips = 0;
