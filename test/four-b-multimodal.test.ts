// cua-s1-4b's multimodal adapter against upstream (fixtures/cua-s1-4b-0.1-multimodal.json and its screenshots, from
// `export/four_b/fixtures.py --modality multimodal`): the chat text, Qwen's image preprocessing and the mRoPE
// positions always; with the bundle in public/models/cua-s1-4b-0.1-multimodal (not in git), the token ids and the
...
import { CuaS1FourB, renderChat, type FourBContext, type FourBManifest, type FourBOption } from "../src/four-b.ts";
...
interface Fixture {
  id: string; family: string; app: string; gold: string[]; chat: string; input_ids: number[]; probs: number[];
...
const fx: { run: string; fixtures: Fixture[] } = JSON.parse(readFileSync(new URL("cua-s1-4b-0.1-multimodal.json", dir), "utf8"));
...
const context = (f: Fixture): FourBContext => ({ app: f.app, taskFamily: f.family, screenshot: screenshot(f) });
...
test("preprocess reproduces Qwen's pixel_values", () => {
  ...
  for (const f of fx.fixtures) {
    ...
    // the screenshots are 760 px wide, so the processor resizes them (bicubic, to 768): allow a resampler that is
    // not torchvision's to differ by a rounding step here and there
...
const bundle = new URL("../public/models/cua-s1-4b-0.1-multimodal/", import.meta.url);
...
const skip = manifest ? false : "no model bundle in public/models/cua-s1-4b-0.1-multimodal (export/build_4b_model.sh --multimodal)";
...
test("probabilities match FourBModel through the vision graph", { skip }, async () => {
  ...
  // torchvision's by 1-2 levels on ~0.1% of pixels, which moves this near-flat model by up to ~0.03
  assert.ok(worst <= 0.05, `max |dp| ${worst}`);
  await m.release();
});
